...
(
  ...
  , case
    ...
    as VALUENUM
  ,date_trunc('day',l.CHARTTIME) as CHARTTIME
  from CHARTEVENTS l
  ...
  and l.charttime between b.intime and b.intime + interval '30' day
...
, gcs_final as (
  ...
  , ROW_NUMBER ()
          OVER (PARTITION BY gcs.ICUSTAY_ID, date_trunc('day',gcs.CHARTTIME)
                ORDER BY gcs.GCS
               ) as IsMinGCS
  from gcs
)
select ie.SUBJECT_ID, ie.HADM_ID, ie.ICUSTAY_ID, gs.charttime
...
ORDER BY ie.SUBJECT_ID, ie.ICUSTAY_ID, gs.CHARTTIME;
